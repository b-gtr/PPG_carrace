...
###################################################
def train_ppg_with_vae(resume_checkpoint=None):
    """
    :param resume_checkpoint: Pfad zu einem Checkpoint (str) oder None.
           Wenn nicht None, werden die Modelle + Optimizer eingeladen und
           ab gespeicherter iteration weitertrainiert.
    """
    ...
    # 3) Prüfen, ob wir von einem Checkpoint starten
    start_iteration = 0
    if resume_checkpoint is not None and os.path.isfile(resume_checkpoint):
        print(f"[Resume] Lade Checkpoint: {resume_checkpoint}")
        checkpoint = torch.load(resume_checkpoint, map_location=DEVICE)

        # VAE-Zustand
        vae.load_state_dict(checkpoint["vae_state_dict"])
        vae_trainer.optimizer.load_state_dict(checkpoint["vae_optimizer_state"])

        # ActorCritic-Zustand
        net.load_state_dict(checkpoint["net_state_dict"])
        ppg_trainer.optimizer.load_state_dict(checkpoint["net_optimizer_state"])

        # Iteration
        start_iteration = checkpoint["iteration"]
        print(f"[Resume] Starte ab Iteration: {start_iteration}")
    else:
        print("[Init] Kein Resume oder Datei nicht gefunden. Starte Training von vorne.")

        # a) Buffer füllen mit Zufallsaktionen
    ...
    for iteration in range(start_iteration, MAX_ITERATIONS):
        ...
        if (iteration + 1) % 5 == 0:
            checkpoint = {
                ...
                "vae_optimizer_state": vae_trainer.optimizer.state_dict(),
                "net_state_dict": net.state_dict(),
                "net_optimizer_state": ppg_trainer.optimizer.state_dict(),
            }
            filename = f"checkpoint_iter_{iteration+1}.pth"
            torch.save(checkpoint, filename)
            print(f"[SAVE] Checkpoint gespeichert unter: {filename}")
...
if __name__ == "__main__":
    # Beispiel: ohne Resume
    # train_ppg_with_vae(resume_checkpoint=None)

    # Beispiel: mit Resume (z.B. "checkpoint_iter_25.pth")
    # train_ppg_with_vae(resume_checkpoint="checkpoint_iter_25.pth")

    # Hier kann man in der Praxis z.B. via Command Line Argument arbeiten.
    train_ppg_with_vae(resume_checkpoint=None)
